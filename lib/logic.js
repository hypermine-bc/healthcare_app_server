...
async function donateMoneyFromPharma(tx) {
  ...
  //if(tx.percentage <= 100) {
    if(tx.percentage <= (100-txIqviaAsset.pharmaNoti.percentageCoverd)) {
      ...
      txIqviaAsset.pharmaNoti.insuranceNoti.percentageCoverd = tx.pharmaNoti.percentageCoverd;
      if(txIqviaAsset.pharmaNoti.percentageCoverd == 100) {
        txIqviaAsset.pharmaNoti.TransferStatus = "SUCCESS";
        txIqviaAsset.pharmaNoti.insuranceNoti.TransferStatus = "SUCCESS";
      } 
      const iqviaInsuAssetRegistry =  await getAssetRegistry(NS + '.InsuranceNotification');
      await iqviaInsuAssetRegistry.update(txIqviaAsset.pharmaNoti.insuranceNoti);
...
async function donateMoneyFromCharity(tx) {
  ...
  if(tx.percentage <= (100-txIqviaAsset.charityNoti.percentageCoverd)) {
    ...
    tx.charityNoti.insuranceNoti.percentageCoverd = tx.charityNoti.percentageCoverd;
    tx.charityNoti.pharmaNoti.percentageCoverd = tx.charityNoti.percentageCoverd;
    var donatedetail = factory.newConcept(NS, 'DonateDetail');
    ...
    if(tx.charityNoti.percentageCoverd == 100) {
      tx.charityNoti.TransferStatus = "SUCCESS";
      tx.charityNoti.insuranceNoti.TransferStatus = "SUCCESS";
      tx.charityNoti.pharmaNoti.TransferStatus = "SUCCESS";
    } 
    
    const iqviaInsurAssetRegistry =  await getAssetRegistry(NS + '.InsuranceNotification');
    await iqviaInsurAssetRegistry.update(tx.charityNoti.insuranceNoti);
    
    const iqviaPharmaAssetRegistry =  await getAssetRegistry(NS + '.PharmaNotification');
    await iqviaPharmaAssetRegistry.update(tx.charityNoti.pharmaNoti);
...
async function donateMoneyFromInsurance(tx) {
  ...
  if(tx.percentage <= (100-txIqviaAsset.insuranceNoti.percentageCoverd)) {
    ...
    if( tx.insuranceNoti.percentageCoverd == 100 ) {
      tx.insuranceNoti.TransferStatus = "SUCCESS"
    } else {
      tx.insuranceNoti.TransferStatus = "IN_PROGRESS"
    }
